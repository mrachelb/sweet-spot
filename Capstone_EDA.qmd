...
filtered_d['year'] = pd.to_datetime(filtered_d['date']).dt.year

# 3. Drop duplicate entries to get unique days per 'item_name' and 'year'
unique_days = filtered_d[['item_name', 'year', 'date']].drop_duplicates()

# 4. Group by 'item_name' and 'year' and count unique dates
result = unique_days.groupby(['item_name', 'year']).size().reset_index(name='unique_days_count')

print(pd.DataFrame(result).head(304))
