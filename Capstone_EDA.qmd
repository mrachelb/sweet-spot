...
## Remove the store in Eppendorf from dataset (opened in June 2024)

```{python}
updated_d = updated_d[(updated_d["date"] <= pd.to_datetime("2024-05-31")) &
           (updated_d["store_name"] != "Eppendorf")]
```
...
```{python}
train_idx, test_idx = next(GroupShuffleSplit(n_splits = 1, random_state=100, test_size = 0.25).split(updated_d, groups = updated_d["date"]))
